...
@WebServlet(name = "SvTurnFilterByAttended", urlPatterns = {"/SvTurnFilterByAttended"})
public class SvTurnFilterByAttended extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Turn> allTurns = (List<Turn>) request.getSession().getAttribute("citizenTurnList");

        boolean isAttended = Boolean.parseBoolean(request.getParameter("isAttended"));

        Citizen citizen = (Citizen) request.getSession().getAttribute("citizen");

        List<Turn> citizenTurnList = allTurns.stream()
                .filter(turn -> turn.isAttended() == isAttended)
